...
M.qtype_easyoddname2={
    insert_structure_into_applet : function(Y, slot, numofstereo, moodleroot){
		...
		elem.setAttribute("src", moodleroot+"/question/type/easyoddname/pix/"+trimgroup+".png");
...
/*
    ...
    //   drag.set('innerHTML', 'You dragged: <strong>' + e.drag.get('node').get('innerHTML') + '</strong>');


 	drag.get('parentNode').insertBefore(drag, drag);
...
/*
    var dd = new Y.DD.Delegate({
        container: '#play', //The common container
        nodes: '.list2' //The items to make draggable
        }).plug(Y.Plugin.DDProxy, {
            moveOnEnd: false,
	  //  hideOnEnd: false,
          //  cloneNode: true
