...
/**
 * Unit tests for the easyoddname question definition class.
 *
 * @package    qtype
...
require_once($CFG->dirroot . '/question/type/easyoddname/question.php');
require_once($CFG->dirroot . '/question/engine/simpletest/helpers.php');
require_once($CFG->dirroot . '/question/type/easyoddname/simpletest/helper.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_easyoddname_question_test extends UnitTestCase {
    public function test_compare_string_with_wildcard() {
        // Test case sensitive literal matches.
        $this->assertTrue(qtype_easyoddname_question::compare_string_with_wildcard(
                'Frog', 'Frog', false));
        $this->assertFalse(qtype_easyoddname_question::compare_string_with_wildcard(
                'Frog', 'frog', false));
        $this->assertTrue(qtype_easyoddname_question::compare_string_with_wildcard(
                '   Frog   ', 'Frog', false));
        $this->assertFalse(qtype_easyoddname_question::compare_string_with_wildcard(
    ...
    public function test_is_complete_response() {
		$question = qtype_easyoddname_test_helper::make_a_easyoddname_question();
    ...
    public function test_get_question_summary() {
        $easyoddname = qtype_easyoddname_test_helper::make_a_easyoddname_question();
        $qsummary = $easyoddname->get_question_summary();
        $this->assertEqual('Name an amphibian: __________', $qsummary);
    }

    public function test_summarise_response() {
        $easyoddname = qtype_easyoddname_test_helper::make_a_easyoddname_question();
        $summary = $easyoddname->summarise_response(array('answer' => 'dog'));
        $this->assertEqual('dog', $summary);
    }

    public function test_classify_response() {
        $easyoddname = qtype_easyoddname_test_helper::make_a_easyoddname_question();
        $easyoddname->start_attempt(new question_attempt_step(), 1);

        $this->assertEqual(array(
                new question_classified_response(13, 'frog', 1.0)),
                $easyoddname->classify_response(array('answer' => 'frog')));
        $this->assertEqual(array(
                new question_classified_response(14, 'toad', 0.8)),
                $easyoddname->classify_response(array('answer' => 'toad')));
        $this->assertEqual(array(
                new question_classified_response(15, 'cat', 0.0)),
                $easyoddname->classify_response(array('answer' => 'cat')));
        $this->assertEqual(array(
                question_classified_response::no_response()),
                $easyoddname->classify_response(array('answer' => '')));
    }
}
